...
(define (pascal n)
  (if (= n 1)
      `((1))
      (append (pascal (- n 1)) (list (append (map (lambda (num) (combinations335 (- n 1) num)) (range (- n 1))) (list 1))))
      ) 
  )

(define (combinations335 n k)
...
(define (create-mapping keys vals)
    (if (not (= (length keys) (length vals)))
      (raise "The lists are not of equal length.")
      (if (not (andmap symbol? keys))
          (raise "The keys are not all symbols.")
          (lambda (x) (list-ref vals (list-index x keys)))
          )
      )  
  )
...
(define list-index
  (lambda (e lst)
    (if (null? lst)
        (raise (string-append "Could not find mapping for symbol " (symbol->string e)))
        (if (eq? (car lst) e)
            0
