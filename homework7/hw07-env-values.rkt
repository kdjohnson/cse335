...
(define (proc-val->proc proc)
  (or (expressed-val? proc) (invalid-args-exception "proc-val->proc" "expressed-val?" proc))
  (cases expressed-val proc
    (proc-val (proc) proc)
    (else (invalid-args-exception "proc-val->proc" "expressed-val?" proc))
    )
  )
